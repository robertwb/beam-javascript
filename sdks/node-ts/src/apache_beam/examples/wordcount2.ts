...
class CountElements extends
    beam.PTransform<beam.PCollection<any>, beam.PCollection<any>> {
  expand(input: beam.PCollection<any>) {
    return input.map(e => ({element: e, count: 1}))
        .apply(new CombineBy('element', 'count', new CountFn()));
  }
}

function wordCount(lines: beam.PCollection<string>): beam.PCollection<any> {
  return lines.map((s: string) => s.toLowerCase())
...
async function main() {
  ...
  // 3333
  await new NodeRunner(new RemoteJobServiceClient('localhost:3333'))
      .run(async root => {
        const lines = await root.asyncApply(new textio.ReadFromText(
