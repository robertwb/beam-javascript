...
import { BytesCoder, Context, IterableCoder, KVCoder, StrUtf8Coder, VarIntCoder } from '../src/apache_beam/coders/standard_coders';
...
describe("JavaScript native coders", function() {
    describe("BSON Object Coder", function() {
        ...
        it("encodes and decodes an object properly", function() {
            ...
            const encoded = bsonCoder.encode(inputObject, writer, Context.selfDelimiting);

            const buffer = writer.finish();
            const reader = new BufferReader(buffer);
            assert.deepEqual(
                bsonCoder.decode(reader, Context.selfDelimiting),
                inputObject)
        });
    });
    ...
    describe("general object coder", function() {
        ...
        it("encodes and decodes an object properly", function() {
            ...
            const encoded = objCoder.encode(inputObject, writer, Context.selfDelimiting);

            const buffer = writer.finish();
            const reader = new BufferReader(buffer);
            assert.deepEqual(
                objCoder.decode(reader, Context.selfDelimiting),
                inputObject)
        })
