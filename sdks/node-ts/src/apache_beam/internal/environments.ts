...
function asNewEnvironment(
    env: runnerApi.Environment, urn: string, payload: Uint8Array) {
  return {
    urn: urn,
    payload: payload,
...
export function asExternalEnvironment(
    env: runnerApi.Environment, address: string) {
  return asNewEnvironment(
      env, 'beam:env:external:v1', runnerApi.ExternalPayload.toBinary({
        endpoint: {
          url: address,
          authentication: null!,
        },
...
export function asDockerEnvironment(
    env: runnerApi.Environment, containerImage: string) {
  return asNewEnvironment(
      env, 'beam:env:docker:v1',
